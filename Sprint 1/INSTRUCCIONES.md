...
2. **Muestra el Jupyter Notebook** (documentación educativa)
   ```bash
   jupyter notebook programas/tienda_aurelion.ipynb
   ```

3. **Ten listo el programa de consola** (backup simple)
   ```bash
   python programas/tienda_aurelion.py
   ```
...
- ✅ Asegúrate de que `datos/tienda_aurelion.csv` existe
- ✅ Si ejecutas desde programas/, las rutas ya están configuradas
