...
El archivo `tienda_aurelion.csv` ya está listo para importar a Power BI. Asegúrate de que contenga:
...
2. Navega hasta `tienda_aurelion.csv`
3. Click en **"Abrir"**
4. Verifica la vista previa de datos
5. Click en **"Cargar"** o **"Transformar datos"** si necesitas ajustes

### Paso 3: Verificar Tipos de Datos

En la vista de **"Modelo"** o **"Datos"**, asegúrate de que:
- `id` sea tipo **Número entero**
- `precio` sea tipo **Número entero** o **Número decimal**
- `stock` sea tipo **Número entero**
- `nombre`, `categoria`, `descripcion`, `proveedor` sean tipo **Texto**

Para cambiar tipo de dato:
1. Click derecho en la columna
2. Selecciona "Cambiar tipo"
...
**Métricas a mostrar:**
- **Total de Productos**: `COUNT(id)`
- **Valor Total Inventario**: `SUM(precio * stock)`
- **Stock Total**: `SUM(stock)`
- **Categorías Únicas**: `DISTINCTCOUNT(categoria)`
- **Productos con Stock Bajo**: `COUNTROWS(FILTER(Productos, [stock] <= 20))`
